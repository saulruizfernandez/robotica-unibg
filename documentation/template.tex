...
\begin{enumerate}
  ...
  \item Find the contours of the arrows based on the retrieved edges from the previous step. Taking only the external ones in order to avoid detecting more than once the same arrow (RETR\_EXTERNAL). Parameter CHAIN\_APPROX\_SIMPLE compresses horizontal, vertical, and diagonal segments and leaves only their end points.
  \begin{figure} % Single column figure
    \includegraphics[width=\linewidth]{contour.png}
    \caption{Contours calculated out of edge detection}
    \label{fig:tcanther}
  \end{figure}
  \item From this step onwards each contour will be processed as a potential arrow…
  \item Out of the contour (vector of 2D coordinates) we calculate the approximate polygon containing all its key points (corners of the arrows).
  \item The convex hull is obtained out of the approximate polygon, as a list of the indexes that form the convex hull.
  \item 1st ARROW CONDITION CHECK (Figures 4 and 5): convex hull contains 5 points (for the first type of arrow), or convex hull contains 4 points (for the second type of arrow). Also, the approximated polygon must have exactly two more points than the convex hull, as is a condition that is always verified for all types of arrows.
  
  \begin{figure} % Single column figure
    \centering
    \includegraphics[height=5cm, keepaspectratio]{flecha1.png}
    \caption{Key points and convex hull of 1st type of arrow}
    \label{fig:tcanther}
  \end{figure}

  \begin{figure} % Single column figure
    \centering
    \includegraphics[height=5cm, keepaspectratio]{flecha2.png}
    \caption{Key points and convex hull of 2nd type of arrow}
    \label{fig:tcanther}
  \end{figure}

  \item Identify the indexes of the two inner points of the arrow (those excluded from the convex hull) in the vector of points that form the approximated polygon.
  \item Find the tip of the arrow / 2nd ARROW CONDITION CHECK: once we have the indexes of the two inner points, and based on the fact that both of them will be at distance 2 from the tip (inside the vector of points that form the approximated polygon), we can do the following checking to compute the tip:   
  \begin{enumerate}[label*=\arabic*.]
    \item Sum 2 to the index of the first of the two points; subtract 2 from the index of the second point. If the index is the same, then it will be the index of the tip.
    \item If the indexes are not the same, we will subtract 2 to the index of the first of the two points; sum 2 to the index of the second point. If the index is the same, it will be the index of the tip.
    \item If none of the conditions are satisfied, it means that the figure is not an arrow.  
  \end{enumerate}
  \item We get the two bottom points of the arrow simply by adding 3 and 4 to the index of the tip in the approximated polygon list of points. Then, we calculate the middle point of them (for the case of the second type of arrow).
  \item With all these calculations, we have obtained the coordinates of the tip and of the bottom of the arrow, so we can build a vector (figure 6) from which we can calculate the size of the arrow, its center and its orientation.
    
  \begin{figure} % Single column figure
    \centering
    \includegraphics[height=5cm, keepaspectratio]{flecha3.png}
    \caption{Arrow vector and the two points that define it}
    \label{fig:tcanther}
  \end{figure}
